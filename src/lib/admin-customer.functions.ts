...
/**
 * Builds one operational customer directory without trusting email as the
 * ownership key once a request has been assigned to a registered account.
 *
 * Rules:
 * - registered profiles are keyed by user id;
 * - requests with user_id stay with that account even if their email differs;
 * - unclaimed guest requests may roll into a registered customer only when the
 *   email matches that profile;
 * - a request already owned by another user is never absorbed by email alone.
 */
async function loadCustomers(search?: string): Promise<AdminCustomer[]> {
  ...
  const profileKeyByEmail = new Map<string, string>();
  ...
  for (const profile of profiles) {
    ...
    const key = `user:${uid}`;
    profileKeyByEmail.set(email.toLowerCase(), key);
    merge(key, {
      user_id: uid,
  ...
  for (const request of requests) {
    ...
    const key = uid
      ? `user:${uid}`
      : profileKeyByEmail.get(email.toLowerCase()) ?? `guest:${email.toLowerCase()}`;
